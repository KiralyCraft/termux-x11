...
public class LorieView extends SurfaceView implements InputStub {
    ...
    // It is used in native code. Takes ownership of each fd (an already-open real file, or a
    // shared-memory region for non-file blobs). The X server side has no MIME database, so it
    // hands over "application/octet-stream" for files and relies on us to refine it from the name.
    // `generation` identifies this clip on the X server side too (see reopenClipboardItem) -- it
    // isn't a locally-assigned counter.
    void setClipboardFileList(String[] names, String[] mimes, long[] sizes, int[] fds, int generation) {
        ...
        for (int i = 0; i < fds.length; i++) {
            pfds[i] = ParcelFileDescriptor.adoptFd(fds[i]);
            resolvedMimes[i] = resolveMime(mimes[i], names[i]);
        }

        Uri[] uris = ClipboardDocumentsProvider.publishFiles(names, resolvedMimes, pfds, generation, mNativeContext);

        String[] clipMimes = new String[resolvedMimes.length + 1];
        System.arraycopy(resolvedMimes, 0, clipMimes, 0, resolvedMimes.length);
        clipMimes[resolvedMimes.length] = "text/uri-list";

        ClipDescription description = new ClipDescription("X11 clipboard", clipMimes);
        // DocumentsUI (RuntimeDocumentClipper) refuses to paste a file clip without this: it reads
        // "clipper:opType" out of the description's extras to decide copy vs. move, and silently
        // ignores the paste if it's missing.
        ...
        // A very large file selection can push the ClipData past the binder transaction limit
        // (~1 MB total, regardless of actual file sizes -- it's just the Uris/strings being
        // marshaled). This call is reached from native code with no pending-exception handling
        // on that side, so letting it throw here would abort the process on the next JNI call.
        try {
            clipboard.setPrimaryClip(clip);
            markOwnClipboardWrite();
    ...
    // Opens every item of a multi-item (or single generic-file) clip and ships them as-is over
    // the native fd-passing path, instead of slurping bytes through Java. Returns false (without
    // sending anything) if any item can't be opened, so the caller can fall back to the text path.
    ...
    // Blocks (with a timeout) for the X server to hand back a fresh fd for a file-list item it
    // previously sent; returns -1 on failure/timeout/stale generation. Called from
    // ClipboardDocumentsProvider, off the GUI thread -- deliberately not @FastNative/@CriticalNative.
