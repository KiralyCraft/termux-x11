...
/* API-33 Choreographer entry points are resolved at run time.  Termux:X11
 * still supports API 24, so merely linking these symbols would make the APK
 * unloadable on older Android versions. */
typedef struct {
...
static Bool lorieRedraw(__unused ClientPtr pClient, __unused void *closure) {
    ...
    if (lorieChoreographerApi.available && timelineDeadlineNs > 0 &&
        ...
        __atomic_store_n(&pvfb->state->latestFrameTimeline.expectedUs,
                         ...
                         __ATOMIC_RELAXED);
        __atomic_store_n(&pvfb->state->latestFrameTimeline.opportunityUs,
...
void lorieHandlePresentBackendRelease(uint8_t mode,
    ...
    if (dixLookupWindow(&window, present_tag.window, serverClient,
        ...
        present_lorie_send_backend_release_notify(
            ...
            deadline_us, expected_us, opportunity_us, opportunity_msc);
}
...
static void lorieChoreographerFrameCallback(__unused long frameTimeNanos, void *data) {
...
#define LOAD_CHOREOGRAPHER_SYMBOL(member, symbol) \
...
#undef LOAD_CHOREOGRAPHER_SYMBOL
    ...
    if (lorieChoreographerApi.available) {
        ...
        AChoreographer_postFrameCallback(choreographer,
                                         lorieChoreographerFrameCallback,
                                         choreographer);
        log(INFO, "Using compatibility Choreographer callback; exact Present pacing disabled");
    }
}
