...
struct LorieViewResources {
    ...
    // Turns the async EVENT_CLIPBOARD_ITEM_REOPEN_REQUEST/REPLY round trip (see reopenClipboardItem)
    // into a synchronous call from whichever (Binder pool) thread invokes it: xcallback runs on
    // the GUI thread and signals this once a reply arrives.
        ...
        if (read(connFd, &e, sizeof(e)) == sizeof(e)) {
            switch(e.type) {
                ...
                case EVENT_CLIPBOARD_LIST_BEGIN: {
                    ...
                    // count comes off the wire, so a huge/corrupt value must not crash us outright;
                    // still have to drain exactly `count` LIST_ITEM messages either way to keep the
                    // socket framing in sync with the sender, so the loop keeps running, just
                    // discarding fds instead of storing them.
                    bool oom = count > 0 && (!itemFds || !itemNames || !itemMimes || !itemSizes);
                    if (oom)
                    ...
                    for (received = 0; received < count; received++) {
                        ...
                        int itemFd = ancil_recv_fd(connFd);
                        if (itemFd < 0)
                            break;
                    ...
                    if (oom) {
                        ...
                        jmethodID id = env->GetMethodID(env->GetObjectClass(thiz), "setClipboardFileList",
                                                        "([Ljava/lang/String;[Ljava/lang/String;[J[II)V");
                        env->CallVoidMethod(thiz, id, names, mimes, sizes, fds, (jint) e.clipboardListBegin.generation);
...
JNIEXPORT jint JNI_OnLoad(JavaVM *vm, __unused void *reserved) {
    ...
    static JNINativeMethod methods[] = {
            ...
            {"sendClipboardEvent", "(JLjava/lang/String;[B)V", (void *) +[](JNIEnv *env, __unused jobject thiz, jlong ptr, jstring mime, jbyteArray data) {
                ...
                // EVENT_CLIPBOARD_SEND's wire format has no mime field -- it always means
                // text/plain -- so anything else has to go via EVENT_CLIPBOARD_SEND_BLOB instead,
                // whose event does carry one, or the real mime is lost on the X server side.
            ...
            // Blocks (with a timeout) waiting for the X server to answer with a fresh fd for a
            // previously-sent file-list item -- see EVENT_CLIPBOARD_ITEM_REOPEN_REQUEST. Called
            // from a Binder pool thread (ClipboardDocumentsProvider.openDocument), never the GUI
            // thread that xcallback/reopenCond's signaler runs on, so this can't deadlock itself.
